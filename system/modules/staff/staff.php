...
$sqla="SELECT * FROM staff s,staff_role r WHERE  r.st_r_id=s.st_r_id ORDER BY s.st_id DESC";
...
$sql="SELECT * FROM staff s,staff_role r WHERE  r.st_r_id=s.st_r_id ORDER BY s.st_id DESC limit $page1,5";
...
var res=confirm("Do You Want to"+msg+" staff Account"); // parameter passing dalete, activate,deactivate
if(res){
  return true;
}else{
	return false;
...
<body>
    <div id="newmain">
       ...
       <a href='addstaff.php'><button type="button" class="btn btn-warning">Add</button></a>
       &nbsp;&nbsp;
...
<button type="button" class="btn btn-warning">Activate</button>    </a>
  ...
  <?php for($i=1;$i<=$nopage;$i++){?>
    <li><a href="staff.php?page=<?php echo $i ?>">
	<?php echo $i; ?></a></li>
    <?php } ?>
  </ul>
  </nav>
