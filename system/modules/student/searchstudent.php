...
$sqla="SELECT * FROM student s AND (s.s_fname LIKE '$search%' or s.s_lname LIKE '$search%' or s.s_NIC LIKE '$search%') ORDER BY s.s_id DESC";
...
$sql="SELECT * FROM student AND (s.s_fname LIKE '$search%' or s.s_lname LIKE '$search%' or s.s_NIC LIKE '$search%')ORDER BY s.s_id DESC limit $page1,5";
...
<body>
    <div id="newmain">
       ...
       <div id="newcontent">
       <h2 align="center">Student Search</h2>
       <hr />
       ...
       <form action="searchstudent.php" method="post">Serach :
...
<button type="button" class="btn btn-success">Activate</button>    </a>
  ...
  <?php for($i=1;$i<=$nopage;$i++){?>
    <li><a href="searchstudent.php?page=<?php echo $i ?> &search=<?php echo $search; ?>">
	<?php echo $i; ?></a></li>
    <?php } ?>
  </ul>
  </nav>
